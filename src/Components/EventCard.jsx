...
function EventCard(props) {
  return (
    <Link to={`/event/${props.id}`} className='space-y-2 p-4 rounded-4xl hover:scale-105'>
      <img src={props.img || heroBg} className="h-80 rounded-xl object-cover w-full" alt={props.title} />
      ...
      <div className="bg-purple-300/50 rounded-xl p-4 space-y-4">
        ...
        <h1 className='text-sm'>{new Date(props.date).toLocaleString()}</h1>
        {props.categories.length > 0 && (
          <div className="space-x-2 md:space-x-4">
            {props.categories.map((cat, idx) => (
              <span key={idx} className="px-3 py-1 bg-purple-400 rounded-4xl">{cat}</span>
            ))}
          </div>
        )}
      </div>
